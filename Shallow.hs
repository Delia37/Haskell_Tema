...
rectangle width height = \(x, y) ->
    let halfWidth = width / 2
        halfHeight = height / 2
    in -halfWidth <= x && x <= halfWidth && -halfHeight <= y && y <= halfHeight
...
circle :: Float -> Region
circle radius = \(x, y) ->
    let distanceFromCenter = sqrt (x^2 + y^2)
    in distanceFromCenter <= radius
...
plot width height region = intercalate "\n" [row y | y <- reverse [(-height)..height]]
  where
    row y = [if region (fromIntegral x, fromIntegral y) then '*' else '.' | x <- [(-width)..width]]
{-
