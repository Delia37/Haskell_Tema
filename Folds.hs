...
instance Show RegionAST where
    show region = go 0 region
      where
        go :: Int -> RegionAST -> String
        go indent (R shape) = case shape of
            FromPoints points     -> replicate (2 * indent) ' ' ++ "FromPoints " ++ show points
            Rectangle width height -> replicate (2 * indent) ' ' ++ "Rectangle " ++ show width ++ " " ++ show height
            Circle radius          -> replicate (2 * indent) ' ' ++ "Circle " ++ show radius
            Complement subRegion   -> replicate (2 * indent) ' ' ++ "~\n" ++ go (indent + 1) subRegion
            Union reg1 reg2        -> replicate (2 * indent) ' ' ++ "+\n" ++ go (indent + 1) reg1 ++ "\n" ++ go (indent + 1) reg2
            Intersection reg1 reg2 -> replicate (2 * indent) ' ' ++ "*\n" ++ go (indent + 1) reg1 ++ "\n" ++ go (indent + 1) reg2
            Transform t reg        -> replicate (2 * indent) ' ' ++ show t ++ "\n" ++ go (indent + 1) reg
...
instance Num RegionAST where
    ...
    (+) = union
    (*) = intersection
    region1 - region2 = region1 `intersection` complement region2
...
instance Functor TransformationShape where
    ...
    fmap f (Combine as) = Combine (map f as)
...
-}
countt :: TransformationAST -> Int
countt = foldTransformationAST transformationCombiner
  where
    transformationCombiner (Translation _ _) = 1
    transformationCombiner (Scaling _) = 1
    transformationCombiner (Combine counts) = sum counts
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner (Complement count)  = count
    combiner (Union count1 count2) = (fst count1 + fst count2, snd count1 + snd count2)
    combiner (Intersection count1 count2) = (fst count1 + fst count2, snd count1 + snd count2)
    combiner (Transform t count) = (fst count, snd count + countt t)
